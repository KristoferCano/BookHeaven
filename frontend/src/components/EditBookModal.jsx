...
function EditBookModal({ isOpen, onClose, onEdit, Books }) {
    ...
    useEffect(() => {
        if (Books) {
            setFormData({
                title: Books.title || '',
                match: Books.match || 85,
                age: Books.age || '16+',
                duration: Books.duration || '120 min',
                genres: Books.genres || ['Drama'],
                description: Books.description || '',
                poster: Books.poster || '',
                backdrop: Books.backdrop || ''
            })
        }
    }, [Books])

    if (!isOpen || !Books) return null
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:8000/api/libros/${Books.id}`, {
                method: 'PUT',
                body: data
            })
        ...
        <div className="modal-overlay">
            <div className="modal modal--edit">
                <div className="modal__header">
                    <h2 className="modal__title">Editar Libro</h2>
